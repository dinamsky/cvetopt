...
if(COption::GetOptionString("eshop", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "samovar_odegda_brands";
		...
		{
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/_index.php", array("brends_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}

$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH."/xml/".LANGUAGE_ID."/99_brends.xml"; 
$iblockType = "shop_samovar_catalog"; 
$iblockCode = "samovar_odegda_brands";
...
CWizardUtil::ReplaceMacros($_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/templates/eshop_bootstrap_/footer.php", array("brands_IBLOCK_ID" => $iblockID));
