...
if(COption::GetOptionString("eshop", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockType = "shop_samovar_catalog";
		$iblockCode = "samovar_odegda_tavar_dnya";
		
		$rsIBlock = CIBlock::GetList(array(), array("XML_ID" => $iblockCode, "TYPE" => $iblockType));
		$iblockID = false; 
		if ($arIBlock = $rsIBlock->Fetch())
		{
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/_index.php", array("TOVAR_ONE_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}

$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH."/xml/".LANGUAGE_ID."/99_tovar_one.xml"; 
$iblockType = "shop_samovar_catalog"; 
$iblockCode = "samovar_odegda_tavar_dnya";
...
if($iblockID == false)
{
	...
	$arFields = Array(
		...
		"CODE" => "samovar_odegda_tavar_dnya", 
		"XML_ID" => $iblockCode,
...
CWizardUtil::ReplaceMacros($_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/templates/".WIZARD_TEMPLATE_ID."_/footer.php",  array("TOVAR_ONE_IBLOCK_ID" => $iblockID));
