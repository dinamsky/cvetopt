...
if(COption::GetOptionString("eshop", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "shop_samovar_priem_".WIZARD_SITE_ID;
		$iblockType = "shop_samovar_content";
		
		$rsIBlock = CIBlock::GetList(array(), array("XML_ID" => $iblockCode, "TYPE" => $iblockType));
		$iblockID = false; 
		if ($arIBlock = $rsIBlock->Fetch())
		{
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/_index.php", array("priem_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}

$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH."/xml/".LANGUAGE_ID."/04_priem.xml"; 
$iblockType = "shop_samovar_content"; 
$iblockCode = "shop_samovar_priem_".WIZARD_SITE_ID;
...
WizardServices::IncludeServiceLang("priem.php", $lang);
//CUserOptions::SetOption("form", "form_element_".$iblockID, array ( 'tabs' => 'edit1--#--'.GetMessage("WZD_OPTION_priem_1").'--,--ACTIVE--#--'.GetMessage("WZD_OPTION_priem_2").'--,--ACTIVE_FROM--#--'.GetMessage("WZD_OPTION_priem_3").'--,--NAME--#--'.GetMessage("WZD_OPTION_priem_5").'--,--CODE--#--'.GetMessage("WZD_OPTION_priem_6").'--,--PREVIEW_TEXT--#--'.GetMessage("WZD_OPTION_priem_8").'--,--DETAIL_TEXT--#--'.GetMessage("WZD_OPTION_priem_10").'--;--', ));
...
CWizardUtil::ReplaceMacros($_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/templates/".WIZARD_TEMPLATE_ID."_/footer.php",   array("priem_IBLOCK_ID" => $iblockID));
