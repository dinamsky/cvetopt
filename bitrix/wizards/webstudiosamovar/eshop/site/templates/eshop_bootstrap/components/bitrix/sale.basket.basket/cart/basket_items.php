		...
		<table id="basket_items">
			<thead>
				<tr>
					...
					if ($arHeader["id"] == "NAME"):
						?>
					<td class="item" colspan="2" id="col_<?=$arHeader["id"];?>" valign=middle>
						<?
					...
					<tbody>
								...
								if ($bPropsColumn):
									...
									if (is_array($arItem["SKU_DATA"]) && !empty($arItem["SKU_DATA"])):
										...
										<td class="custom">
											...
											<div class="centered">
												<table cellspacing="0" cellpadding="0" class="counter" style="background: #ddd; border: 1px solid #ddd;">
													<tr>
														<td>
															<?
															...
															<a href="javascript:void(0);" class="minus" onclick="setQuantity(<?=$arItem["ID"]?>, <?=$arItem["MEASURE_RATIO"]?>, 'down', <?=$useFloatQuantityJS?>);" style="margin: 0 5px;"><i class="fa fa-minus"></i></a>
														<? endif;?>
															...
															<?=$max?>
															step="<?=$ratio?>"
															style="max-width: 50px; text-align: center; background: white;"
															...
															<?if (floatval($arItem["MEASURE_RATIO"]) != 0	):?>
															<a href="javascript:void(0);" class="plus" onclick="setQuantity(<?=$arItem["ID"]?>, <?=$arItem["MEASURE_RATIO"]?>, 'up', <?=$useFloatQuantityJS?>);" style="margin: 0 5px;"><i class="fa fa-plus"></i></a>
														<? endif;?>
										...
										<td class="custom">
											...
											if ($arHeader["id"] == "SUM"):
												?>
											<div class="summa_basket" id="sum_<?=$arItem["ID"]?>">
												<?
												endif;
										...
										<td class="control">
											...
											if ($bDeleteColumn):
												?>
											<a class="btn btn-info btn-sm btn_car" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>"><i class="fa fa-trash-o"></i> <?=GetMessage("SALE_DELETE")?></a>
											<?
											endif;
											if ($bDelayColumn):
												?>
											<a class="btn btn-info btn-sm btn_car"href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?>"><i class="fa fa-level-down"></i> <?=GetMessage("SALE_DELAY")?></a>
											<?
											endif;
											?>
										</td>
						...
						<div class="bx_ordercart_order_pay">
							...
							<div class="bx_ordercart_order_pay_left" id="coupons_block">
								...
								if ($arParams["HIDE_COUPON"] != "Y")
								{
									...
									<div class="bx_ordercart_coupon">
										<span><?=GetMessage("STB_COUPON_PROMT")?></span><input type="text" id="coupon" name="COUPON" value="" onchange="enterCoupon();">&nbsp;<a class="btn btn-danger btn-sm" href="javascript:void(0)" onclick="enterCoupon();" title="<?=GetMessage('SALE_COUPON_APPLY_TITLE'); ?>"><?=GetMessage('SALE_COUPON_APPLY'); ?></a>
									</div><?
							...
							<div class="bx_ordercart_order_pay_center text-center">
								...
								{
									?>
									<a class="btn btn-danger btn-md" href="javascript:void(0)" onclick="updateBasket();"><?=GetMessage("SALE_REFRESH")?></a>
									<?
								}
								?>
								<?if(isset($_SESSION['url_back'])):?>
								<a class="btn btn-danger btn-md" href="<?=$_SESSION['url_back']?>"><?=GetMessage("BACK_SHOP")?></a>
								<?endif?>
								<a class="btn btn-danger btn-md" href="javascript:void(0)" onclick="checkOut();"><?=GetMessage("SALE_ORDER")?></a>
							</div>
						</div>
					</div>
					<?
