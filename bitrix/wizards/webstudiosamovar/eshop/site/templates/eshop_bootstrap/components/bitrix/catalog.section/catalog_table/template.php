...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	?>
			<? // ---------------------   Шильдики?>
			<div id="<? echo $arItemIDs['STICKER_ID']; ?>" class="bx_stick average left top" title="<? echo $arItem['LABEL_VALUE']; ?>" style="display: none;">
...
?>
	...
	<? if( $s_all == 0):?>
	<div class="nalichie_n"><i class="demo-icon icon-hourglass-o"></i> <?=getmessage('nal_n')?></div>
	<?else:?>
	<div class="nalichie_y"><i class="demo-icon icon-check"></i> <?=getmessage('nal_y')?></div>
	<?endif?>
...
<?=$minPrice['PRINT_DISCOUNT_VALUE'];?>
	...
	<a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" class="btn btn-default"><?=getmessage("more")?> <i class="demo-icon icon-angle-right"></i></a>


	</td>

<td valign="top" style=" padding: 19px 0 0 10px;">
...
<div class="tt cart" el="<?=$arItem['ID']?>" <?if(!$arParams['USE_PRODUCT_QUANTITY']):?>style=" width: 100%;"<?endif?>>
