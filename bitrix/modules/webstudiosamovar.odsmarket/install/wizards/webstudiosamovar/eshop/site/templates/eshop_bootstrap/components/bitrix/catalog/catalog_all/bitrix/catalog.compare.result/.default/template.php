...
$APPLICATION->SetTitle(GetMessage("SRAVN_TITLE"));
...
?><div class="bx_sort_container hidden-xs">
...
{
?>
<div class="bx_filtren_container hidden-xs">
	...
	if (!empty($arResult["ALL_FIELDS"]))
	{
		foreach ($arResult["ALL_FIELDS"] as $propCode => $arProp)
		{
			...
			{
		?>
		<li style="display: none;"><span onclick="CatalogCompareObj.MakeAjaxAction('<?=CUtil::JSEscape($arProp["ACTION_LINK"])?>')">
...
<table class="data-table">
	...
	<tr>
		<td style="background: white; padding-bottom: 0;"></td>
		<?foreach($arResult["ITEMS"] as &$arElement)
		{
		?>
		<td style="background: white; padding-bottom: 0;">
...
if (!empty($arResult["SHOW_FIELDS"]))
{
	foreach ($arResult["SHOW_FIELDS"] as $code => $arProp)
	{
		...
		if ($showRow)
		{
			?><tr><td <?if($code == "DETAIL_PICTURE"):?> style="/*text-align: center;*/ vertical-align: middle; background: white;"<?endif?>>
			
			<?if($code !== "DETAIL_PICTURE" && $code !== "NAME"):?><?=GetMessage("IBLOCK_FIELD_".$code)?><?endif?>

			</td><?
			foreach($arResult["ITEMS"] as &$arElement)
			{
		?>
				<td valign="top" <?if($code == "DETAIL_PICTURE"):?> style="/*text-align: center;*/ vertical-align: middle; background: white;"<?endif?>>
		<?
				switch($code)
				{
					case "NAME":
						?>
						<div class="name"><a href="<?=$arElement["DETAIL_PAGE_URL"]?>"><?=$arElement[$code]?></a></div>
						<?
						break;
...
<tr>
	<td style=" background: white;"><?//=GetMessage('CATALOG_COMPARE_PRICE');?></td>
	<?
	foreach ($arResult["ITEMS"] as &$arElement)
	{
		if (isset($arElement['MIN_PRICE']) && is_array($arElement['MIN_PRICE']))
		{
			?><td style=" background: white;"><strong><? echo $arElement['MIN_PRICE']['PRINT_DISCOUNT_VALUE']; ?></strong></td><?
		}
		else
		{
...
<tr>
	...
	foreach($arResult["ITEMS"] as &$arElement)
	{
		...
		<td>
			...
			<noindex><a class="btn btn-default btn-sm" href="<?=$arElement["DETAIL_PAGE_URL"]?>" rel="nofollow"> <?=GetMessage("CATALOG_COMPARE_BUY"); ?></a></noindex>
			<?elseif(!empty($arResult["PRICES"]) || is_array($arElement["PRICE_MATRIX"])):?>
			<span class="no_sk"><?=GetMessage("CATALOG_NOT_AVAILABLE")?></span>
			<?endif;?>
		</td>
		<?}?>
	</tr>
