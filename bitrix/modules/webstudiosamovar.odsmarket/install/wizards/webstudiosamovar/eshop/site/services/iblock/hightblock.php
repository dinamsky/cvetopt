...
$dbHblock = HL\HighloadBlockTable::getList(
	array(
		"filter" => array("NAME" => "Samovarproizvoditel")
	)
);
if (!$dbHblock->Fetch())
{
	$data = array(
		'NAME' => 'Samovarproizvoditel',
		'TABLE_NAME' => 'b_hlbd_samovarproizvoditel',
	);

	$result = HL\HighloadBlockTable::add($data);
	$ID = $result->getId();

	$_SESSION["HBLOCK_HAR_ID"] = $ID;
	...
	foreach ($arUserFields as $arFields)
	{
		...
		{
			WizardServices::IncludeServiceLang("hightblock.php", $languageID);
