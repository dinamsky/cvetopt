...
if(COption::GetOptionString("eshop", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "shop_samovar_akcii_".WIZARD_SITE_ID;
		...
		{
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/_index.php", array("akcii_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}

$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH."/xml/".LANGUAGE_ID."/02_akcii.xml"; 
$iblockType = "shop_samovar_content"; 
$iblockCode = "shop_samovar_akcii_".WIZARD_SITE_ID;
...
WizardServices::IncludeServiceLang("akcii.php", $lang);


CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/akcii/index.php", array("akcii_IBLOCK_ID" => $iblockID));
CWizardUtil::ReplaceMacros($_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/templates/".WIZARD_TEMPLATE_ID."_/footer.php",  array("akcii_IBLOCK_ID" => $iblockID));
