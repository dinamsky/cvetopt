...
if(COption::GetOptionString("eshop", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "shop_samovar_plitki_".WIZARD_SITE_ID;
		...
		{
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/_index.php", array("plitki_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}

$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH."/xml/".LANGUAGE_ID."/05_plitki.xml"; 
$iblockType = "shop_samovar_content"; 
$iblockCode = "shop_samovar_plitki_".WIZARD_SITE_ID;
...
WizardServices::IncludeServiceLang("plitki.php", $lang);
//CUserOptions::SetOption("form", "form_element_".$iblockID, array ( 'tabs' => 'edit1--#--'.GetMessage("WZD_OPTION_plitki_1").'--,--ACTIVE--#--'.GetMessage("WZD_OPTION_plitki_2").'--,--ACTIVE_FROM--#--'.GetMessage("WZD_OPTION_plitki_3").'--,--NAME--#--'.GetMessage("WZD_OPTION_plitki_5").'--,--CODE--#--'.GetMessage("WZD_OPTION_plitki_6").'--,--PREVIEW_TEXT--#--'.GetMessage("WZD_OPTION_plitki_8").'--,--DETAIL_TEXT--#--'.GetMessage("WZD_OPTION_plitki_10").'--;--', ));
...
CWizardUtil::ReplaceMacros($_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/templates/".WIZARD_TEMPLATE_ID."_/footer.php",  array("plitki_IBLOCK_ID" => $iblockID));
